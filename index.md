...
We checked. **NVIDIA NeMo Guardrails / Colang** ([colang docs](https://docs.nvidia.com/nemo/guardrails/colang_2/overview.html)) ships a declarative DSL for agent flows, rails, and fallbacks since 2023 — closest prior art. Colang's primitives are *flows* (turn-by-turn dialogue paths) and *rails* (input/output filters); SSL v7 composes a different stack: character + scope-as-code + audit-chain-as-code + adversarial-as-code in a single sovereign spec, with a SHA-256 forensic chain Colang does not provide and inheritance / weight ordering Colang does not have. The two are complementary — Colang for flow scaffolding, SSL v7 for character + safety perimeter + audit. **GuardrailsAI** is safety post-hoc on output, not pre-flight on input. **Letta** is memory. **Marvin** is decorators around prompts. **DSPy** ([arxiv:2310.03714](https://arxiv.org/abs/2310.03714)) compiles task signatures, a different layer entirely. None of them compose all of: scope-as-code + adversarial-as-code + audit-chain-as-code into a single sovereign spec with inheritance + lifecycle. If we are wrong about this and a project predates SSL with the same composition, open an issue and we will cite it here. The space is open.
